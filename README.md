...
The anonymizer writes one directory per anonymized sample with public-safe metadata, transformed snippet files, a transform summary, and a hashed mapping file. Every anonymized sample directory contains both `vulnerable.*` and `fixed.*`. Accepted positive samples preserve their vulnerable/fixed pair; accepted negative samples also export both files, with the anonymized `vulnerable.*` and `fixed.*` copies intentionally matching so the sample still has a fixed reference. The anonymized sample directories are shuffled together with a stable default seed, and you can override it with `--seed`. Use `--dry-run` to preview work and `--force` to regenerate existing anonymized outputs.
